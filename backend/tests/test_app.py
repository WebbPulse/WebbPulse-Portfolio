...
def test_health_reports_unhealthy_database(client: TestClient, monkeypatch):
    ...
    db_client.dynamodb_resource().meta.client.delete_table(TableName="webbpulse-test-site-content")
...
def test_trailing_slash_variants_do_not_redirect(client: TestClient, path, test_site_content):
...
def test_rate_limit_endpoints_are_gone(client: TestClient, admin_auth_headers):
    ...
    assert client.get("/api/v1/admin/rate-limit/stats", headers=admin_auth_headers).status_code == 404
