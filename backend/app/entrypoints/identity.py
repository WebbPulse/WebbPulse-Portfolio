...
`POST /api/v1/admin/login`, and nothing else. One route on its own function, so
the signing key is reachable from exactly one place.

Run by the image as `python -m app.entrypoints.identity`. There is no Lambda
